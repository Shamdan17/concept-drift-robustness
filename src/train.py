...
def get_args_parser():
    ...
    parser.add_argument('--train_start_date', default=201801, type=int, help='year in 4 digits followed by month in 2 digits')
    ...
    parser.add_argument('--test_end_date', default=202002, type=int, help='year in 4 digits followed by month in 2 digits')
...
def main(args):
    ...
    X_train, X_val, y_train, y_val = train_test_split(train_set.features, train_set.labels, test_size=0.20, random_state=0) 
    ...
    print('Accuracy on validation data (same time period):',accuracy)

    preds = model.predict(test_set.features)
    accuracy = accuracy_score(test_set.labels, preds)
    print('Accuracy on test data (different time period):',accuracy)
